...
for jSubj = 1:length(SUBJS) %929 % [73 128 450 528 544 569 586 621 720 834 929]%834% 1:length(SUBJS)
    ...
    SESSIONS = dir([D filesep SUBJS(jSubj).name]);
    ...
    for jSession=1:length(SESSIONS)
        ...
        try
            if(~and(and(length(dtiQA)>1,length(Stamper)==1),length(MultiAtlas)==1))
                ...
                fa1 = loaduntouchniiorniigz([fa1name ]);
                fa2 = loaduntouchniiorniigz([fa2name ]);
                mdM = loaduntouchniiorniigz([mdMname ]);
                md1 = loaduntouchniiorniigz([md1name ]);
                md2 = loaduntouchniiorniigz([md2name ]);
                
                adM = loaduntouchniiorniigz(adMname);
                ad1 = loaduntouchniiorniigz(ad1name);
                ad2 = loaduntouchniiorniigz(ad2name);
                rdM = loaduntouchniiorniigz(rdMname);
                rd1 = loaduntouchniiorniigz(rd1name);
                rd2 = loaduntouchniiorniigz(rd2name);
                eve = loaduntouchniiorniigz(eveName);
                bc = loaduntouchniiorniigz(brainColorName);
                
                roi1 = loaduntouchniiorniigz([roi1name ]);
                roi2 = loaduntouchniiorniigz([roi2name ]);
                ...
                if(mean(mean(mean((fa2.img-faM.img).^2)))>mean(mean(mean((flipdim(fa2.img,2)-faM.img).^2))))
                  ...
                  fp = fopen('/fs4/masi/landmaba/BLSAdti/BLSA/headerCheckv8.txt','at');
                fprintf(fp,'%s %f %f %f %f %f %f %f %f %f %f %f %f %f %f %f\n',SESSIONS(jSession).name,bc.hdr.dime.pixdim(2),bc.hdr.dime.pixdim(3),bc.hdr.dime.pixdim(4),eve.hdr.dime.pixdim(2),eve.hdr.dime.pixdim(3),eve.hdr.dime.pixdim(4),...
                    fa1.hdr.dime.pixdim(2),fa1.hdr.dime.pixdim(3),fa1.hdr.dime.pixdim(4),...
                    fa2.hdr.dime.pixdim(2),fa2.hdr.dime.pixdim(3),fa2.hdr.dime.pixdim(4),...
                    faM.hdr.dime.pixdim(2),faM.hdr.dime.pixdim(3),faM.hdr.dime.pixdim(4));
                fclose(fp)
